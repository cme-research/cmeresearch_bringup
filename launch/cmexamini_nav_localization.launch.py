...
def generate_launch_description():
    """cmexa-mini nav2 + slam_toolbox (LOCALIZATION mode).

    Mirror of cmexaiii_nav_localization.launch.py pointed at config/cmexamini/.
    Loads a previously saved map from the volume-mounted /maps directory.
    """
    declared_arguments = []
    ...
    # In localization mode the nav2 map comes from the volume-mounted /maps dir.
    # Save a map there first with: ros2 run nav2_map_server map_saver_cli -f /maps/cmexamini_house
    declared_arguments.append(
        DeclareLaunchArgument(
            "nav_map_filepath",
            default_value='/maps/cmexamini_house.yaml',
            description="Path to nav2 map yaml file (volume-mounted)",
        )
    )

    # slam_params_localization.yaml sets mode:localization and map_file_name:/maps/cmexamini_house
    declared_arguments.append(
        DeclareLaunchArgument(
            "slam_config_filepath",
            default_value=launch.substitutions.TextSubstitution(text=os.path.join(
                get_package_share_directory('cmeresearch_bringup'),
                'config/cmexamini/slam_params_localization.yaml')),
